...
"""
get_ipython().system('{sys.executable} -m pip install gym')
get_ipython().system('{sys.executable} -m pip install gym_super_mario_bros==7.3.0 nes_py')
get_ipython().system('{sys.executable} -m pip install torch torchvision torchaudio')
...
# Test du jeu avec une IA qui choisit aléatoirement les actions à effectuer

# In[4]:


#Paramètres modifiables par l'utilisateur pour le test
No_Images = 100 #Nombre d'images à parcourir avant que la simulation ne prenne fin.
...
# Prétraitement de l'environnement : les images du jeu
...
# Actions
# A chaque état, en fonction du ${\epsilon}$ de la stratégie ${\epsilon}$-greedy, l'agent peut choisir d'effectuer l'action la plus optimale selon sa table Q (on dit qu'il exploite) ou une action totalement aléatoire (on dit qu'il explore)
